...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < amountGasStation + 1; i++)
    {
        ...
        struct GasStation *parameter = (struct GasStation *)malloc(sizeof(struct
                                                                          GasStation));
        *parameter = data;
...
void *gasStation(void *argument)
{
    ...
    for (i = 0; i < Station->amountCars; i++)
    {
        ...
        struct GasStation *dataCar = (struct GasStation *)malloc(sizeof(struct
                                                                        GasStation));
        *dataCar = *Station;
...
{
    struct GasStation *Station = (struct GasStation *)argument;
    struct GasStation *Result = (struct GasStation *)malloc(sizeof(struct
                                                                   GasStation));
    pthread_mutex_lock(&mutexCar);
    if (Station->price < Station->holdings)
    {
        Station->holdings -=
            int(Station->price * (100 - Station->profitMargins) / 100);
